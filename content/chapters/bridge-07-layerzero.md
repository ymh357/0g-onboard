...
## 1. LayerZero 是 0G 代币的实际跨链标准

在前面的章节中，我们学习了 CCIP（第 4 章）和 Wormhole（第 5-6 章）的技术原理。现在我们要揭示一个**链上已验证的关键事实**：

> **0G 原生代币的跨链标准不是 CCIP，而是 LayerZero OFT。**
>
> 以太坊上的 0G 代币合约名称为 `ZeroGravityOFT`（Solidity v0.8.22），地址 `0x4b948d64de1f71fcd12fb586f4c776421a35b3ee`，直接继承了 LayerZero 的 OFT 标准。合约已配置三链 Peer：Ethereum (eid=30101) ↔ BSC (eid=30102) ↔ 0G (eid=30388)，形成全链 Burn/Mint 网络。
>
> 来源：Etherscan 验证源码 + 链上 `peers()` 查询

这意味着 LayerZero 不是"第三个补充协议"——**它是 0G 生态中最核心的跨链基础设施**。

| 能力 | LayerZero | CCIP | Wormhole |
| :--- | :--- | :--- | :--- |
| **0G 原生代币跨链** | **已确定（OFT 合约已部署）** | hub.0g.ai 有 CCIP 集成代码，但非原生代币标准 | - |
| **NFT 跨链标准** | **ONFT 标准（最成熟）** | 无专用标准 | NFT Bridge 模块 |
| **自助部署** | **完全自助，无需审批** | 需与 Chainlink 协调 | 项目方自主 |
| **链覆盖** | **70+ 链（最广）** | EVM 链 | 30+ 链 |
| **0G 主网部署** | **V2 完整部署（Endpoint + DVN + Executor）** | 存在集成代码 | 文档已支持（Chain ID 67），链上部署待确认 |
| **0G 上已有代币** | 0G (OFT) + stgWETH/stgUSDT/stgUSDC (Stargate) | - | - |

LayerZero 在 0G 生态中承担了三个角色：
1. **原生代币跨链**：0G 代币本身就是 OFT 合约，跨链必须通过 LayerZero
2. **稳定币桥接**：Stargate（LayerZero 生态的 DeFi 桥）已在 0G 上部署了 stgWETH/stgUSDT/stgUSDC
3. **iNFT 跨链**：LayerZero 的 ONFT 标准是 ERC-7857 跨链的最佳候选方案

> **参考来源**：
> - [LayerZero 0G Mainnet Deployment](https://docs.layerzero.network/v2/deployments/chains/og)
> - [Etherscan: ZeroGravityOFT Contract](https://etherscan.io/address/0x4b948d64de1f71fcd12fb586f4c776421a35b3ee#readContract)
...
| 职责 | 实际方案 | 依据 |
| :--- | :--- | :--- |
| **0G 原生代币跨链** | **LayerZero OFT（已确定）** | 合约 `ZeroGravityOFT` 已部署，Peer 已配置（链上验证） |
| **稳定币/主流资产** | **Stargate（LayerZero 生态）** 为主，CCIP / NTT 可作补充 | stgWETH/stgUSDT/stgUSDC 已在 0G 链上；CCIP 集成代码存在于 hub.0g.ai |
| **iNFT 跨链** | **LayerZero ONFT** | 最成熟的 NFT 跨链标准，已部署 0G 主网 |
| **生态项目自助跨链** | **LayerZero OFT** | 完全自助部署，无需审批，上线最快 |
| **高安全场景/消息传递** | **CCIP**（作为补充） | 三网验证 + RMN 适合高价值资产或跨链消息 |

---

## 本章小结

- **LayerZero 是 0G 生态中最核心的跨链基础设施**：0G 原生代币合约 `ZeroGravityOFT` 直接继承 OFT 标准（链上已验证）；稳定币通过 Stargate 桥接；iNFT 可用 ONFT 标准。
